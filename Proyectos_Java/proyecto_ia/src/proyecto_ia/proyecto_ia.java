...
public class proyecto_ia {
 static Stack tributos = new Stack(); 
    ...
    private static boolean Verificar_atentado(char[][] matriz, int nx, int ny, int fila, int colu) {
        if ( ((nx+1<fila)&&(ny+1<colu)&&(matriz[nx+1][ny]=='T')) || ((nx-1>=0)&&(ny-1>=0)&&(matriz[nx-1][ny]=='T')) || ((nx-1>=0)&&(ny+1<colu)&&(matriz[nx][ny+1]=='T'))|| ((nx+1<fila)&&(ny-1>=0)&&(matriz[nx][ny-1]=='T')) ){
            return true;
        } else {
            return false;
        }
        ...
        */
        ...
        if (muerto == true || muerta==true  || esquinas_y_rodeados==true ) {
            ...
            if (min== -1) {
             ...
             private static boolean esquinas_y_rodeados(char[][] matriz, int fila, int colu) {
                 if(matriz[0][0] == 'T' && matriz[0][1] == '*' && matriz[1][0] == '*'){ return true;}
                 if(matriz[fila-1][colu-1] == 'T' && matriz[fila-2][colu-1] == '*' && matriz[fila-1][colu-2] == '*'){ return true;}
                 if(matriz[0][colu-1] == 'T' && matriz[0][colu-2] == '*' && matriz[1][colu-1] == '*'){ return true;}
                 if(matriz[fila-1][0] == 'T' && matriz[fila-1][1] == '*' && matriz[fila-2][0] == '*'){ return true;}
    ...
    private static int BFS(int posfilak, int poscoluk, int fila, int colu) {
        ...
        while( !Q.isEmpty()){               //Mientras cola no este vacia
            ...
            for( int i = 0 ; i < 4 ; i++ ){     //Recorremos hasta 4 porque tenemos 4 posibles adyacentes
                ...
                if( nx >= 0 && nx < fila && ny >= 0 && ny < colu && (visitado[nx][ny]==false) && matriz[ nx ][ ny ] != '*' && matriz[ nx ][ ny ] == '.' && !Verificar_atentado(matriz, actual.x, actual.y, fila, colu) && !Verificar_vecinos(matriz, nx, ny, fila,colu)){
