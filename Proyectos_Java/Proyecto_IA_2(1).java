...
public class Proyecto_IA {
    ...
    private static boolean Verificar_atentado(char[][] matriz, int nx, int ny, Integer fila, Integer colu) {
        if ( ((nx+2<fila)&&(matriz[nx+2][ny]=='T')) || ((nx-2>=0)&&(matriz[nx-2][ny]=='T')) || ((ny+2<colu)&&(matriz[nx][ny+2]=='T')) || ((ny-2>=0)&&(matriz[nx][ny-2]=='T')) ){
            return true;
        } else {
            return false;
        }
    ...
    private static int BFS(Integer posfilak, Integer poscoluk, Integer fila, Integer colu) {
        ...
        /*for (int w2=0; w2<fila; w2++){
            for (int w1=0; w1<colu; w1++){
		...
		while( !Q.isEmpty()){				//Mientras cola no este vacia
			...
			for( int i = 0 ; i < 4 ; i++ ){		//Recorremos hasta 4 porque tenemos 4 posibles adyacentes
				...
				if( nx >= 0 && nx < fila && ny >= 0 && ny < colu && !visitado[ nx ][ ny ] && matriz[ nx ][ ny ] != '*' && matriz[ nx ][ ny ] == '.' && !Verificar_atentado(matriz, nxaux, nyaux, fila, colu) && !Verificar_vecinos(matriz, nx, ny, fila,colu)){
